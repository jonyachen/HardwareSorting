...
`ifndef BSORT_DESIGN1_V
`define BSORT_DESIGN1_V

`include "../vc/vc-StateElements.v"

module bsort_design1
#(
parameter ADDR_WIDTH = 8, // byte address to index into all BRAM lines (size of startAddr)
...
bsort_design1Dpath#(ADDR_WIDTH, COL_WIDTH, REQ_MSG_SZ) dpath
(
.clk                (clk),
.go			(go),
.msgsSent           (msgsSent),
...
reg [ADDR_WIDTH-1:0] currAddr = 8'b0;
...
count <= count + 1'b1;
memReadRequest <= {1'b0, currAddr, 2'b00, 32'd0};
end
else if (go_reg && (count != numElem_reg)) begin
currAddr <= currAddr + 8'd4;
count <= count + 1'b1;
memReadRequest <= {1'b0, currAddr, 2'b00, 32'd0};
end
